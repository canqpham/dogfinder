  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const [currentBreed, nextBreed] = await Promise.all([
        getApi<BreedResponse[], { limit: number; page: number, order: string, has_breed: boolean }>(
          "/images/search",
          { limit: 1, page: page ? parseInt(page) : 1, order: "ASC", has_breed:true }
        ),
        getApi<BreedResponse[], { limit: number; page: number, order: string, has_breed: boolean }>(
          "/images/search",
          { limit: 1, page: page ? parseInt(page) + 1 : 2, order: "ASC", has_breed:true }
        ),
      ]);
      if (!currentBreed) {
